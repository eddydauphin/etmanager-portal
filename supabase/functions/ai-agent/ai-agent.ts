...
async function executeTool(supabase: any, name: string, params: any, userId: string, profile: any) {
  ...
  switch (name) {
    ...
    case 'get_coaching': {
      // Check coaching where user is trainee
      const { data: asTrainee } = await supabase.from('development_activities').select('title, status, start_date, coach:coach_id(full_name)').eq('trainee_id', userId).in('status', ['pending', 'in_progress'])
      // Check coaching where user is coach
      const { data: asCoach } = await supabase.from('development_activities').select('title, status, start_date, trainee:trainee_id(full_name)').eq('coach_id', userId).in('status', ['pending', 'in_progress'])
      
      if (!asTrainee?.length && !asCoach?.length) return { success: true, result: 'No active coaching sessions.' }
      
      let r = ''
      if (asTrainee?.length) {
        r += `**Your coaching (as trainee):**\n`
        asTrainee.forEach((c: any, i: number) => r += `${i + 1}. ${c.title} - Coach: ${c.coach?.full_name}\n`)
      }
      if (asCoach?.length) {
        r += `\n**Sessions you're coaching:**\n`
        asCoach.forEach((c: any, i: number) => r += `${i + 1}. ${c.title} - Trainee: ${c.trainee?.full_name}\n`)
      }
      return { success: true, result: r }
    }
